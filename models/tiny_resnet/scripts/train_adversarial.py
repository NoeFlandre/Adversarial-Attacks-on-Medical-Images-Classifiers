...
def evaluate_batch(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels in dataloader:
            ...
            # Ensure consistent precision when using CUDA
...
def train_model_adversarial(model, train_dataset, val_dataset, epochs=10, batch_size=64, 
                           lr=1e-4, device='cpu', logger=None, epsilon=0.05, mix_ratio=0.5):
    """
    ...
    for epoch in range(epochs):
        ...
        for images, labels in train_dataloader:
            ...
            optimizer.zero_grad()
            # Ensure consistent data types by casting to full precision
            if torch.cuda.is_available():
                with torch.amp.autocast('cuda', enabled=False):
        ...
        scheduler.step(val_metrics_clean['loss'])
        ...
        if early_stopping(val_metrics_clean['loss'], model):
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(range(1, epochs_trained + 1), clean_val_accuracies, marker='o', label='Clean Validation Accuracy')
    plt.plot(range(1, epochs_trained + 1), adv_val_accuracies, marker='o', label='Adversarial Validation Accuracy')
...
def evaluate_adversarial_accuracy(model, dataset, epsilon, batch_size=64, device='cpu'):
    ...
    for images, labels in dataloader:
        ...
        # Generate adversarial examples using consistent precision
        adv_images = fgsm_attack.generate(images, labels)
        
        # Evaluate with consistent precision
        if torch.cuda.is_available():
            with torch.amp.autocast('cuda', enabled=False):
                with torch.no_grad():
