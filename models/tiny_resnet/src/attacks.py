...
class FGSM:
    ...
    def generate(self, images, labels, targeted=False):
        ...
        # Forward pass - ensure consistent data types
...
def evaluate_attack(model, dataset, attack, batch_size=64, device='cpu', logger=None, save_results=True):
    ...
    for i, (images, labels) in enumerate(dataloader):
        ...
        with torch.no_grad():
            # Use consistent precision when CUDA is available
            if torch.cuda.is_available():
                with torch.amp.autocast('cuda', enabled=False):
                    # Clean predictions
...
class AdversarialDataset(Dataset):
    ...
    def __init__(self, original_dataset, attack, device='cpu'):
        ...
        for images, labels in dataloader:
            images, labels = images.to(device), labels.to(device)
            # Ensure consistent precision when generating adversarial examples
